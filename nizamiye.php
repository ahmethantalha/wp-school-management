...
require_once NIZAMIYE_DIR . 'includes/class-nizamiye-sheet.php';
require_once NIZAMIYE_DIR . 'includes/class-nizamiye-alerts.php';
...
register_activation_hook( __FILE__, array( 'Nizamiye_Install', 'activate' ) );
...
add_action( 'admin_init', function () {
	if ( get_option( 'nizamiye_db_version' ) !== NIZAMIYE_VERSION ) {
		Nizamiye_Install::activate();
	}
}, 0 );
 ...
 */
function nizamiye_is_limited_user() {
	return is_user_logged_in() && current_user_can( 'nizamiye_access' ) && ! current_user_can( 'manage_options' );
}
...
add_action( 'admin_bar_menu', function ( $wp_admin_bar ) {
	...
	if ( in_array( 'nizamiye_teacher', $roles, true ) ) {
		$role_label = nizamiye_is_class_teacher( $user->ID ) ? 'Sınıf Öğretmeni' : 'Öğretmen';
	} elseif ( in_array( 'nizamiye_parent', $roles, true ) ) {
		$role_label = 'Veli';
	} elseif ( in_array( 'nizamiye_student', $roles, true ) ) {
		$role_label = 'Öğrenci';
	} else {
		$role_label = '';
	}
...
add_action( 'admin_init', function () {
	if ( ! nizamiye_is_limited_user() || wp_doing_ajax() ) {
		return;
	}
	global $pagenow;
...
// wp_add_inline_style() ile enqueue edilir (bkz. wp_enqueue commands rehberi).
add_action( 'admin_enqueue_scripts', function () {
	if ( ! nizamiye_is_limited_user() ) {
		return;
	}
	wp_register_style( 'nizamiye-admin-bar', false, array(), NIZAMIYE_VERSION );
	wp_enqueue_style( 'nizamiye-admin-bar' );
	wp_add_inline_style( 'nizamiye-admin-bar', '
