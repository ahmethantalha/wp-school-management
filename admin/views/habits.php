...
$habits  = $term_id ? SMS_Habits::for_term( $term_id, $teacher ? get_current_user_id() : 0 ) : array();
?>
<div class="wrap sms-wrap">
	<?php sms_view_header( 'Alışkanlıklar', 'Kitap okuma, sabah namazına kalkış gibi alışkanlıklar tanımlayın ve günlük takip edin.' ); ?>

	<?php if ( $term_id ) : ?>
		<div class="sms-toolbar">
			<span></span>
			<a href="<?php echo esc_url( admin_url( 'admin.php?page=sms-habits&view=edit&sms_term=' . $term_id ) ); ?>" class="sms-btn sms-btn-primary"><span class="dashicons dashicons-plus-alt2"></span> Yeni Alışkanlık</a>
		</div>
	<?php endif; ?>
	...
	<?php if ( $habits ) : ?>
		<div class="sms-class-grid">
			<?php foreach ( $habits as $h ) :
				$rate    = SMS_Habits::completion_rate( (int) $h->id );
				...
				<div class="sms-card sms-class-card">
					...
					<div class="sms-progress">
						<div class="sms-progress-bar <?php echo esc_attr( sms_rate_class( null !== $rate ? (int) $rate : null ) ); ?>" style="width:<?php echo null !== $rate ? (int) $rate : 0; ?>%"></div>
					</div>
					...
					<div class="sms-class-card-actions">
						<a class="sms-btn sms-btn-primary sms-btn-sm" href="<?php echo esc_url( admin_url( 'admin.php?page=sms-habits&view=track&habit_id=' . (int) $h->id . '&sms_term=' . $term_id ) ); ?>">Takip Doldur</a>
						<a class="sms-btn sms-btn-ghost sms-btn-sm" href="<?php echo esc_url( admin_url( 'admin.php?page=sms-habits&view=edit&habit_id=' . (int) $h->id . '&sms_term=' . $term_id ) ); ?>">Düzenle</a>
					</div>
				</div>
			<?php endforeach; ?>
		</div>
